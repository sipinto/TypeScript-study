...
## 타입 변수에 담아 사용 (with - type 키워드) & readonly

### 타입 정의가 너무 길면 Type Aliases(별칭) 사용
- 타입 별칭 사용
  - 사용 법

    -> type 타입변수명 = 타입종류(단일, 다중 가능);
...
- const 변수
  - 용도
    ...
    -> 값이 변하지 않는 변수를 만들고 싶을 때 const 사용
    
    -> 재할당 시 에러 발생하기 때문에 값이 변하는 것 미리 감지하여 차단 가능
  - 예외

    -> object 자료를 const에 할당해도 object 내부의 값은 변경 가능

    -> const는 재할당만 차단, 속성 변경은 가능 
- readonly 키워드
  - 사용
  
    -> const에 할당된 object 속성 변경 차단할 때 사용
  - 사용법
  
    -> 속성 왼쪽에 붙일 수 있음

    -> 사용 시 특정 속성을 변경 불가능하게 잠금
### 속성 몇개가 선택사항 이라면
- 속성을 선택사항으로 구현 하려면
  - 방법
  
    -> type alias 여러개 생성이 아닌 `?` 연산자 추가

### type 키워드 여러개 병합 가능
- 타입 병합
  - 방법
    -> or 연산자 이용하여 Union type 생성 가능
    
    -> object에 지정한 경우 병합도 가능

    -> &기호 사용 시 object 안의 두 개의 속성 병합 

    -> 일반 자료형 & object 도 가능

### type 키워드는 재정의가 불가능
- 예외
  - interface 키워드 사용 시 재정의 가능 
